...
// Mirror of SemanticPoint layout written by semantic_cloud_node / semantic_cloud_nodelet.
// Must stay byte-for-byte identical to the struct in semantic_cloud_node.cpp.
...
// Returns true if msg has the exact layout we expect (verified once per unique cloud source).
...
private:
    ...
    void mapPublishCallback(const ros::TimerEvent&) {
        // getCellViews() copies only (x,y,z,class,confidence) per cell — ~20 bytes
        // vs ~250 bytes for MapCell. Skipped entirely when no subscribers.
        const bool need_cells = map_pub_.getNumSubscribers() > 0
                             || (publish_uncertainty_map_
                                 && uncertainty_map_pub_.getNumSubscribers() > 0);
        std::vector<hesfm::SemanticMap::CellView> cells;
        if (need_cells) {
    ...
    bool convertPointCloud(const sensor_msgs::PointCloud2::ConstPtr& msg,
        ...
        // ── Fast path: known packed layout from our own cloud node ──────────────
        if (!cloud_layout_checked_) {
            cloud_layout_fast_ = isKnownLayout(*msg);
            cloud_layout_checked_ = true;
            ROS_INFO("[mapper] cloud layout: %s", cloud_layout_fast_ ? "fast (direct cast)" : "generic (field scan)");
        }
    ...
    void publishSemanticMap(const std::vector<hesfm::SemanticMap::CellView>& cells) {
        if (map_pub_.getNumSubscribers() == 0 || cells.empty()) return;

        // Write directly into PointCloud2 — avoids pcl::PointCloud allocation + toROSMsg copy.
        // Layout: x(f32) y(f32) z(f32) rgb(u32) — 16 bytes/point, matches rviz PointCloud2 RGB.
        ...
        for (size_t i = 0; i < cells.size(); ++i) {
            ...
            out[i].y = cell.y;
            out[i].z = cell.z;
            const int cls = static_cast<int>(cell.pred_class);
            if (cls >= 0 && cls < ncol) {
    ...
    void publishUncertaintyMap(const std::vector<hesfm::SemanticMap::CellView>& cells) {
        if (uncertainty_map_pub_.getNumSubscribers() == 0 || cells.empty()) return;

        // Write directly — avoids pcl::PointCloud allocation + toROSMsg copy.
        // Layout: x(f32) y(f32) z(f32) intensity(f32) — 16 bytes/point.
