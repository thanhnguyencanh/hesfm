...
private:
    ...
    void setupSubscribers() {
        ...
        if (use_uncertainty_) {
            ...
            sync3_->registerCallback(boost::bind(&SemanticCloudNode::syncCallback3,
                                                 this, _1, _2, _3));
            ROS_INFO("  Sync: 3-topic (depth + semantic + uncertainty)");
        } else {
            ...
            sync2_->registerCallback(boost::bind(&SemanticCloudNode::syncCallback2,
                                                 this, _1, _2));
            ROS_INFO("  Sync: 2-topic (depth + semantic), uncertainty=%.2f", default_uncertainty_);
        }
    }
    ...
    void generateCloud(const cv::Mat& depth,
        ...
        // Pre-fill PointCloud2 fields (constant across frames)
        ...
        // Allocate worst-case directly in cloud_msg.data — write points in-place, no memcpy
        const int max_pts = out_w * out_h;
        cloud_msg.data.resize(static_cast<size_t>(max_pts) * sizeof(SemanticPoint));
        SemanticPoint* out_ptr = reinterpret_cast<SemanticPoint*>(cloud_msg.data.data());
        uint32_t count = 0;
        ...
        // Shrink to actual point count (no reallocation — only adjusts size metadata)
        cloud_msg.data.resize(static_cast<size_t>(count) * sizeof(SemanticPoint));
